...
public class JobInProgress {
	...
	/*** Added by Xin - 04/09/2012 ***/
	// A hashmap of pairs of the map and the host where it is scheduled.
	// The map objects are indexed by integers.
	HashMap<Integer, String> mapTargetLocs = new HashMap<Integer, String>();

	// Added by Xin - 05/18/2012
	/*
	 * HashMap<Integer, ArrayList<String>> mapSplitLocs = new HashMap<Integer,
	 * ArrayList<String>> ();
	 */

	// Added by Xin 05/06/2012
	// For calculate each region's computation workload.
	...
	private int numOfRegionAlongX = 0;
	private int numOfRegionAlongY = 0;
	...
	private void checkTaskLimits() throws IOException {
		...
		LOG.info(jobId + ": nMaps=" + numMapTasks + " nReduces="
				+ numReduceTasks + " max=" + maxTasks);
	...
	private Map<Node, List<TaskInProgress>> createCache(
		...
		for (int i = 0; i < splits.length; i++) {
			...
			// Added by Xin - 05/18/2012
			// ArrayList<String> splitLocs = new ArrayList<String>();
	...
	private int CalculateOutDegree(int idx) {
		...
		if (idx < numOfRegionAlongX
				|| idx > (numOfRegionAlongX * (numOfRegionAlongY - 1))
				|| idx % numOfRegionAlongX == 0
				|| idx % numOfRegionAlongX == numOfRegionAlongX - 1) {
			return 4;
		}
	...
	public synchronized void initTasks() throws IOException,
		...
		if (numMapTasks != splits.length) {
			throw new IOException(
					"Number of maps in JobConf doesn't match number of "
							+ "recieved splits for job " + jobId + "! "
							+ "numMapTasks=" + numMapTasks + ", #splits="
							+ splits.length);
		}
		...
		for (int i = 0; i < numMapTasks; ++i) {
			...
			// Added by Xin - 04/28/2012
			maps[i].setOutDegree(CalculateOutDegree(i));
		}
		...
		// Create reduce tasks
		//
		/*** Begin commented by Xin - 04/09/2012 ***/
		/*
		 * this.reduces = new TaskInProgress[numReduceTasks]; for (int i = 0; i
		 * < numReduceTasks; i++) { reduces[i] = new TaskInProgress(jobId,
		 * jobFile, numMapTasks, i, jobtracker, conf, this, numSlotsPerReduce);
		 * nonRunningReduces.add(reduces[i]); }
		 */
		/*** End commented by Xin - 04/09/2012 ***/

		/*** Begin added by Xin - 04/09/2012 ***/

		numReduceTasks = numMapTasks;
		conf.setNumReduceTasks(numMapTasks);
		this.reduces = new TaskInProgress[numReduceTasks];
		for (int i = 0; i < numReduceTasks; ++i) {
			reduces[i] = new TaskInProgress(jobId, jobFile, numReduceTasks, i,
					jobtracker, conf, this, numSlotsPerReduce);
		}

		// zOrderTasks(reduces, xNum, yNum);

		for (int i = 0; i < numReduceTasks; ++i) {
			nonRunningReduces.add(reduces[i]);
		}

		/*** End added by Xin - 04/09/2012 ***/
	...
	public synchronized void updateTaskStatus(TaskInProgress tip,
		...
		if (change) {
			...
			if (state == TaskStatus.State.SUCCEEDED) {
				...
				// For mammoth
				// @Ze
				//	TODO:collect finish time, in millsec
				this.jobtracker.getSkewMonitor().collectFinishTime(tip.isMapTask(),
			...
			if (taskEvent != null) {
				...
				if (ttStat != null) { // ttStat can be null in case of lost
										// tracker
	...
	public synchronized Task obtainNewMapTask(TaskTrackerStatus tts,
		...
		int target = findNewMapTask(tts, clusterSize, numUniqueHosts,
				anyCacheLevel, status.mapProgress());
		if (target == -1) {
			return null;
		}
		/* Added by Xin - 04/11/2012 ** */
		mapTargetLocs.put(target, tts.getTrackerName());
		LOG.info("Log by xin: map " + target + " is scheduled to "
				+ tts.getHost());
	...
	public synchronized Task obtainNewLocalMapTask(TaskTrackerStatus tts,
		...
		/* Added by Xin - 04/11/2012 ** */
		// mapTargetLocs.put(target, tts.getHost());
		mapTargetLocs.put(target, tts.getTrackerName());
		LOG.info("Log by xin: map " + target + " is scheduled to "
				+ tts.getHost());
	...
	public synchronized Task obtainNewNonLocalMapTask(TaskTrackerStatus tts,
		...
		// TODO
		//	set wlAmount of task
		result.setObAbmount(maps[target].wlAmount);
	...
	// Added by Xin - 05/06/2012
	// Order the arrays of maps and splits into Z scanning order

	private void zOrderTasks(TaskInProgress[] tips, int xNum, int yNum) {
		int[] idxArray = new int[xNum * yNum];
		zScanOrder(idxArray, xNum, yNum);
		TaskInProgress[] tmpTips = new TaskInProgress[tips.length];
		for (int i = 0; i < tips.length; ++i) {
			tmpTips[i] = tips[idxArray[i]];
		}
		for (int i = 0; i < tips.length; ++i) {
			tips[i] = tmpTips[i];
		}
	...
	private void zOrderSplits(TaskSplitMetaInfo[] splits, int xNum, int yNum) {
		int[] idxArray = new int[xNum * yNum];
		zScanOrder(idxArray, xNum, yNum);
		TaskSplitMetaInfo[] tmpSplits = new TaskSplitMetaInfo[splits.length];
		for (int i = 0; i < splits.length; ++i) {
			tmpSplits[i] = splits[idxArray[i]];
		}
		for (int i = 0; i < splits.length; ++i) {
			splits[i] = tmpSplits[i];
		}
	}

	private void zScanOrder(int[] idxArray, int xNum, int yNum) {

		int[] xPivotArray = new int[xNum + yNum - 1];
		int[] yPivotArray = new int[xNum + yNum - 1];

		int xPivot = 0, yPivot = 0;
		for (int i = 0; i < xNum + yNum - 1; ++i) {
			xPivotArray[i] = xPivot;
			if (i < xNum - 1) {
				xPivot += 1;
			} else {
				xPivot += xNum;
		...
		for (int i = 0; i < xNum + yNum - 1; ++i) {
			...
			int distance = 0;
			while (xPivotArray[i] + distance < yPivotArray[i]) {
				distance += (xNum - 1);
				if (i % 2 == 1) {
					p = p + xNum - 1;
				} else {
	...
	// Added by Xin - 05/06/2012
	// Comparator for comparing task's computation workload.
	private class TaskWorkloadComparator implements Comparator<TaskInProgress> {

		public int compare(TaskInProgress task1, TaskInProgress task2) {
			if (task1.wlAmount < task2.wlAmount) {
				return 1;
			} else if (task1.wlAmount == task2.wlAmount) {
				return 0;
			} else {
				return -1;
			}
	...
	private synchronized TaskInProgress findTaskFromList(
		...
		// Added by Xin - 04/28/2012
		// find the tip who has the maximum outdegree in tips,
		// and schedule it firstly.
		int maxOutDegree = 0, minOutDegree = 100;
		// int maxWorkload = -1;
		Iterator<TaskInProgress> iterForFirst = tips.iterator();
		while (iterForFirst.hasNext()) {
			TaskInProgress tip = iterForFirst.next();
			int tipIdx = tip.getIdWithinJob();
			tip.wlAmount = obBuilder.getRegionWorkload(tipIdx, pBuilder);
			if (tip.isMapTask() && tip.getOutDegree() > maxOutDegree) {
				maxOutDegree = tip.getOutDegree();
			}
			if (tip.isMapTask() && tip.getOutDegree() < minOutDegree) {
				minOutDegree = tip.getOutDegree();
			}
			/*
			 * if (tip.isMapTask() && tip.wlAmount > maxWorkload) { maxWorkload
			 * = tip.wlAmount; }
			 */
		}

		// Added by Xin - 04/30/2012
		// Schedule method 4:
		// Random
		//List<TaskInProgress> tipsInList = new ArrayList<TaskInProgress>(tips);
		//Collections.shuffle(tipsInList);
		//Iterator<TaskInProgress> iter = tipsInList.iterator();

		// Schedule method : slide window
		Iterator<TaskInProgress> iter = tips.iterator();

		// Added by Xin - 05/06/2012
		// Schedule method 5:
		// sort map tasks according to their workloads,
		// and schedule heavy task early.
		// Collections.sort(tipsInList, new TaskWorkloadComparator());
		...
		while (iter.hasNext()) {
			...
			if (tip.isRunnable() && !tip.isRunning()) {
				...
				if (!tip.hasFailedOnMachine(ttStatus.getHost())
					...
					/*** Added by Xin - 04/11/2012 ***/
					// Select a tip (reduce task) if
					// 0. co-locate with a processing/processed map
					if (!tip.isMapTask()) { // Reduce task
						String mapTaskLoc = mapTargetLocs.get(tip
								.getIdWithinJob());
						if (mapTaskLoc != null
								&& mapTaskLoc.compareTo(ttStatus.getTrackerName()) == 0
								&& canReduceScheduled(tip.getIdWithinJob(),
										mapTargetLocs.keySet()) == true
										) {
							LOG.info("Log by xin: reduce "
									+ tip.getIdWithinJob()
									+ " is scheduled to " + ttStatus.getHost());
							iter.remove();
							return tip;
						} else {
							/*
							LOG.info("Log by xin: corresponding map "
									+ tip.getIdWithinJob()
									+ " has not been processed yet.");
							*/
							// Commented by Xin - 04/29/2012
							// return null;
							/*
							LOG.info("Log by xin: current there are "
									+ tips.size() + " reduce tasks.");
							*/
						}
					}

					// Schedule method 1:
					// Schedule map tasks according to the index sequence
					else { // Map task
						/*
						 * // wait until a heartbeat from the right host arrives
						 * LOG.info("Log by xin: ttStatus.getHost() " +
						 * ttStatus.getHost());
						 * LOG.info("Log by xin: checking tip " +
						 * tip.getIdWithinJob());
						 * 
						 * if ( isTtStatusFromSplitLocs( ttStatus.getHost(),
					...
					/*
					 * else { // Map task if (tip.getOutDegree() == minOutDegree
					 * || iter.hasNext() == false) { iter.remove();
					 * LOG.info("Log by xin: the scheduled task's workload is "
					 * + tip.wlAmount); return tip; } }
					 */

					/*
					 * // Schedule method 5: // sort map tasks according to
					 * their workloads, // and schedule heavy task early. else {
					 * // Map task if (tip.wlAmount == maxWorkload ||
					 * iter.hasNext() == false) { iter.remove();
					 * LOG.info("Log by xin: the scheduled task's workload is "
					 * + tip.wlAmount); return tip; } }
					 */
	...
	/*
	 * private boolean isTtStatusFromSplitLocs( String ttStatusHost,
	 * ArrayList<String> splitLocs) { for (int i=0; i<splitLocs.size(); ++i) {
	 * LOG.info("Log by xin: splitLocs[i] " + splitLocs.get(i)); if
	 * (splitLocs.get(i).indexOf(ttStatusHost) != -1) { return true; } } return
	 * false; }
	 */
	...
	private boolean canReduceScheduled(int reduceIndex, Set<Integer> mapKeys) {
		...
		if (reduceIndex % numOfRegionAlongX != 0) {
			if (!mapKeys.contains(reduceIndex - 1)) {
				return false;
			}
		}
		// right neighbor exists
		if ((reduceIndex % numOfRegionAlongX) != (numOfRegionAlongX - 1)) {
			if (!mapKeys.contains(reduceIndex + 1)) {
				return false;
			}
		}
		// top neighbor exists
		if (reduceIndex - numOfRegionAlongX > 0) {
			if (!mapKeys.contains(reduceIndex - numOfRegionAlongX)) {
				return false;
			}
		}
		// bottom neighbor exists
		if (reduceIndex + numOfRegionAlongX < numOfRegionAlongX
				* numOfRegionAlongY) {
			if (!mapKeys.contains(reduceIndex + numOfRegionAlongX)) {
				return false;
			}
		}
		// and itself's map has been done as well
		if (!mapKeys.contains(reduceIndex)) {
			return false;
		}
