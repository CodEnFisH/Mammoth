...
public abstract class TaskStatus implements Writable, Cloneable {
	...
	// processed item cnter
	private AtomicLong skewInfo = new AtomicLong(0);
	
	// -1 means no skew
	private AtomicLong stopAt = new AtomicLong(-1);
	
	public long getSkewInfo(){
		return skewInfo.get();
	}
	public void setSkewInfo(long info){
		skewInfo.set(info);
	}
	
	public void markAsSkew(long stopAt){
		setStopSign(stopAt);
	}
	...
	public void setStopSign(long stopAt){
