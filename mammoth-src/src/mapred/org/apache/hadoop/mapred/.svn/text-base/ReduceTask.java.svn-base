...
class ReduceTask extends Task {
	...
	private Path[] getMapFiles(FileSystem fs, boolean isLocal)
		...
		if (isLocal) {
			...
			for (int i = 0; i < numMaps; ++i) {
				...
				System.out
						.println("Log by xin: local job's input file for reduce = "
								+ mapOutputFile.getInputFile(i).toString());
			}
		} else {
			// for non local jobs
			for (FileStatus filestatus : mapOutputFilesOnDisk) {
				...
				System.out
						.println("Log by xin: NON local job's input file for reduce = "
	...
	public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
		...
		// Added by xin -04/18/2012
		// numOfRegionAlongX = jobContext.getChunkNumX();
		// numOfRegionAlongY = jobContext.getChunkNumY();
		...
		if (!isLocal) {
			...
			if (!reduceCopier.fetchOutputs()) {
				...
				LOG.info("Log by xin: reduce copier is throwing an exception.");
			...
			JobTracker.LOG
					.info("Log by xin: reduce copier fetched output normally.");
		}

		copyPhase.complete(); // copy is already complete

		JobTracker.LOG.info("Log by xin: copy phase ends.");
		...
		JobTracker.LOG.info("Log by xin: sort phase ends.");
	...
	class ReduceCopier<K, V> implements MRConstants {
		...
		// Added by xin - 04/14/2012
		private final Map<Integer, List<Integer>> finishedMapIndexs = new ConcurrentHashMap<Integer, List<Integer>>();

		private final Map<Integer, List<Integer>> fetchedMapIndexs = new ConcurrentHashMap<Integer, List<Integer>>();
		...
		private class MapOutputCopier extends Thread {
				...
				while (true) {
					try {
						...
						try {
							...
							// Added by xin - 04/16/2012
							synchronized (fetchedMapIndexs) {
								List<Integer> indexs = fetchedMapIndexs
										.get(reduceTask.getPartition());
								if (indexs == null) {
									indexs = Collections
											.synchronizedList(new LinkedList<Integer>());
									fetchedMapIndexs.put(
											reduceTask.getPartition(), indexs);
								}
								indexs.add(new Integer(loc.getTaskAttemptId()
										.getTaskIndex()));
							}
							...
							LOG.info("Log by xin: reduce "
									...
									+ " gets data from map "
									+ loc.getTaskAttemptId().getTaskIndex());
			...
			private long copyOutput(MapOutputLocation loc) throws IOException,
				...
				LOG.info("Log by xin: we begin to copy an output from a map at "
						+ loc.getOutputLocation().toString());
				...
				LOG.info("Log by xin: the copied map output size is " + bytes);
			...
			private MapOutput getMapOutput(MapOutputLocation mapOutputLoc,
				...
				LOG.info("Log by xin: get map output at " + url.toString()
						+ " with decompressed len " + decompressedLength
						+ " and compressed len " + compressedLength);
		...
		public boolean fetchOutputs() throws IOException {
			...
			while (copiedMapOutputs.size() < numMaps && mergeThrowable == null) {
				...
				while (locItr.hasNext()) {
					...
					LOG.info("Log by xin: in retry fetches.");
					...
					if (locList != null) {
						...
						LOG.info("Log by xin: in retry fetches, mapLocation is be fetched "
								+ locList.size());
				...
				synchronized (scheduledCopies) {
					...
					while (hostsItr.hasNext()) {
						...
						/*
						 * LOG.info("Log by xin: the knownOutputsByLoc's size is "
						...
						// Commented out by xin
						/*
						 * //Identify duplicate hosts here if
						 * (uniqueHosts.contains(host)) { numDups +=
						 * knownOutputsByLoc.size(); continue; }
						 */
						...
						/*
						 * LOG.info("Log by xin: the knownOutputsByLoc's size, "
						 * + "after penalty is " + knownOutputsByLoc.size());
						 */
						...
						synchronized (knownOutputsByLoc) {
							...
							while (locItr.hasNext()) {
								...
								LOG.info("Log by xin: current map output loc is "
								...
								if (obsoleteMapIds.contains(loc
									...
									LOG.info("Log by xin: current map output is obsoleted.");

									locItr.remove();
									continue;
								}

								LOG.info("Log by xin: begin to get location.");

								uniqueHosts.add(host);

								// Added by xin - 04/16/2012
								// Only copies from neighbors will be scheduled.
								if (shouldFetched(loc)) {

									scheduledCopies.add(loc);

									LOG.info("Log by xin: "
											+ "one scheduled copy is scheduled to fetch at "
											+ loc.getOutputLocation()
													.toString());
								...
								// Commented out by xin - 04/16/2012
				...
				while (numInFlight > 0 && mergeThrowable == null) {
					...
					if (cr.getSuccess()) { // a successful copy
						...
						// required to make sure of the notification in case of
						// a
						// corner case :
						// when noFailedFetches reached maxFetchRetriesPerMap
						// and
						// reducer reached the end of shuffle, then we may miss
						// sending
						// a notification if the difference between
						// noFailedFetches and fetchRetriesPerMap is not
						// divisible by 2
						...
						// c. send notification immediately if it is a read
						// error and
						// "mapreduce.reduce.shuffle.notify.readerror" set true.
						if ((reportReadErrorImmediately && cr.getError()
								.equals(CopyOutputErrorType.READ_ERROR))
								|| ((noFailedFetches >= fetchRetriesPerMap) && ((noFailedFetches - fetchRetriesPerMap) % 2) == 0)) {
							synchronized (ReduceTask.this) {
						...
						// attempts
						currentTime = System.currentTimeMillis();
						int currentBackOff = noFailedFetches <= fetchRetriesPerMap ? BACKOFF_INIT
								* (1 << (noFailedFetches - 1))
						...
						if (cr.getError()
							...
							if (pendingCopies <= numMaps
									* MIN_PENDING_MAPS_PERCENT) {
				...
				// Added by xin - 04/12/2012
				// Check whether all the map outputs required by current reduce
				// task are fetched. Jump out of the loop if required outputs
				// are copied.
				if (isDependencySatisfied() && numInFlight <= 0) {
					LOG.info("Log by xin: dependency checking returns true.");
					break;
				}
			}

			LOG.info("Log by xin: all required map tasks have finished.");
			...
			LOG.info("Log by xin: all copier have terminated.");
			...
			LOG.info("Log by xin: merging finished.");

			return mergeThrowable == null;
			// return mergeThrowable == null && copiedMapOutputs.size() ==
			// numMaps;
		}

		private boolean shouldFetched(MapOutputLocation loc) {
			int reduceIndex = getPartition();
			int mapIndex = loc.getTaskAttemptId().getTaskIndex();

			LOG.info("Log by xin: checking if should be fetched, "
					+ " reduce index is " + reduceIndex + " map index is "
					+ mapIndex);

			if (mapIndex == reduceIndex) {
				LOG.info("Log by xin: output of map " + mapIndex
						+ " should be fetched. 1");
				return true;
			}

			if (reduceIndex % numOfRegionAlongX != 0
					&& mapIndex - reduceIndex == -1) {
				LOG.info("Log by xin: output of map " + mapIndex
						+ " should be fetched. 2");
				return true;
			}
			...
			if (reduceIndex - numOfRegionAlongX > 0
					&& reduceIndex - mapIndex == numOfRegionAlongX) {
				LOG.info("Log by xin: output of map " + mapIndex
						+ " should be fetched. 4");
				return true;
			}
			...
			if (reduceIndex + numOfRegionAlongX < numOfRegionAlongX
					* numOfRegionAlongY
					&& reduceIndex + numOfRegionAlongX == mapIndex) {
				LOG.info("Log by xin: output of map " + mapIndex
						+ " should be fetched. 5");
				return true;
			}
		...
		// Added by xin - 04/12/2012
		// The reduce task may not require the outputs from all maps,
		// but partially.
		private boolean isDependencySatisfied() {
			int reduceIndex = getPartition();
			synchronized (fetchedMapIndexs) {
				if (fetchedMapIndexs.size() == 0) {
					return false;
				}

				int numOfFinishedMaps = finishedMapIndexs.get(
						reduceTask.getPartition()).size();
				if (numOfFinishedMaps >= numOfRegionAlongX * numOfRegionAlongY) {
					return true;
				}
				/*
				else { 
				...
				*/
				...
				List<Integer> fetchedMapTasks = fetchedMapIndexs
						.get(reduceIndex);

				// left neighbor exists
				if (reduceIndex % numOfRegionAlongX != 0) {
					if (!fetchedMapTasks.contains(reduceIndex - 1)) {
						return false;
					}
				}
				...
				// top neighbor exists
				if (reduceIndex - numOfRegionAlongX > 0) {
					if (!fetchedMapTasks.contains(reduceIndex
							- numOfRegionAlongX)) {
						return false;
					}
				}
			...
			LOG.info("Log by xin: reduce " + reduceIndex
					+ " has collected all its neighbor's results.");
		...
		private void addToMapOutputFilesOnDisk(FileStatus status) {
			synchronized (mapOutputFilesOnDisk) {
				...
				JobTracker.LOG
						.info("Log by xin: one more map output is added.");
		...
		private class GetMapEventsThread extends Thread {
			...
			private int getMapCompletionEvents() throws IOException {
				...
				JobTracker.LOG.info("Log by xin: reduce "
						+ reduceTask.getPartition()
						+ " receives a map completion event.");
				...
				if (update.shouldReset()) {
					...
					LOG.info("Log by xin: mapLocations is tobe cleared "
							+ mapLocations.size());
					...
					LOG.info("Log by xin: mapLocations has been cleared.");
				}
				...
				for (TaskCompletionEvent event : events) {
					...
					case SUCCEEDED: {
						...
						LOG.info("Log by xin: map output is placed at " + host
						...
						LOG.info("Log by xin: after adding, the map output location"
								+ " size is " + mapLocations.get(host).size());

						/*
						 * LOG.info(
						 * "Log by xin: after adding map completion event, " +
						 * "there are " + mapLocations.size() + " hosts and " +
						 * mapLocations.get(host) + " has " +
						 * mapLocations.get(host).size() + " maps.");
						 */

						numNewMaps++;

						// Added by xin - 04/14/2012
						List<Integer> indexs = finishedMapIndexs.get(reduceTask
								.getPartition());
						if (indexs == null) {
							indexs = Collections
									.synchronizedList(new LinkedList<Integer>());
							finishedMapIndexs.put(reduceTask.getPartition(),
									indexs);
						}
						indexs.add(new Integer(event.idWithinJob()));
						JobTracker.LOG.info("Log by xin: reduce "
								+ reduceTask.getPartition()
								+ " has one more map task "
								+ event.idWithinJob() + " finished.");
					}
						break;
					case FAILED:
					case KILLED:
